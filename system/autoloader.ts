...
export default class Autoloader {
    ...
    private async load(name: string):Promise<ImpModule>  {
        ...
        if (this.jetpack.exists(config.path + '.ts') == 'file' || this.jetpack.exists(config.path + '.js') == 'file') {
            ...
            let moduleClass = new moduleConstructor.default(config, this.env);
            this.modules.push(moduleClass);
            this.env.event.emit('imp:module:load', name);
        } else {
            this.env.echo('shocked', `File for the module to load is missing "${this.env.terminal.str.red(config.path)}"`);
            return null;
        }
        this.modulesToLoad--;
        this.env.event.emit('imp:module:load:after', name);
        // final callback
